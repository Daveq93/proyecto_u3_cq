...
public class FacturaRepoImpl implements IFacturaRepo {
	...
	public Factura buscarPorNumero(String numero) {
		TypedQuery<Factura> myQ = this.em.createQuery(
				"Select f FROM Factura f JOIN FETCH f.detallesFactura WHERE f.numero=:numero", Factura.class);
	...
	public List<Factura> buscarPorNumeroInnerJoin(String numero) {
		// TODO Auto-generated method stub
		TypedQuery<Factura> myQ = this.em.createQuery(
				"Select f FROM Factura f JOIN f.detallesFactura df WHERE f.numero =: numero", Factura.class);
	...
	public List<Factura> buscarCantidadMayorDetalleFact(Integer cantidad) {
		// TODO Auto-generated method stub
		TypedQuery<Factura> myQ = this.em.createQuery(
				"SELECT f FROM Factura f LEFT JOIN f.detallesFactura df WHERE df.cantidad > : cantidad", Factura.class);
	...
	public List<Factura> buscarMayoresFecha(LocalDateTime fecha, Integer cantidad) {
		// TODO Auto-generated method stub
		TypedQuery<Factura> myQ = this.em.createQuery(
				"SELECT f FROM Factura f RIGHT JOIN f.detallesFactura df WHERE f.fecha >:fecha and df.cantidad >= : cantidad",
				Factura.class);
	...
	public Factura buscarPorNumeroJinWhere(String numero) {
		// TODO Auto-generated method stub
		TypedQuery<Factura> myQ = this.em.createQuery(
				"SELECT f  FROM Factura f, DetalleFactura df WHERE f = df.factura and f.numero = : numero ",
				Factura.class);
		myQ.setParameter("numero", numero);
	...
	public List<Factura> buscarMayoresFecha(LocalDateTime fecha) {
		// TODO Auto-generated method stub
		TypedQuery<Factura> myQ = this.em.createQuery("SELECT f FROM Factura f JOIN FETCH f.detallesFactura df where f.fecha>=:fecha", Factura.class);
		return myQ.setParameter("fecha", fecha).getResultList();
	}
