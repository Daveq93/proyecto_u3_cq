...
import com.uce.edu.repo.modelo.FacturaDTO;
...
import com.uce.edu.service.IFacturaService;
...
//		}
		...
		// this.transferenciaService.realizarTransferencia("A-111", "A-211", new
		// BigDecimal(20));

		// this.transferenciaService.realizarTransferenciaFachada("A-111", "A-211", new
		// BigDecimal(5));
...
//		this.productoService.insertarProducto(prod4);
		...
		// this.gestorCompraService.generarFactura("000001271","1712134578" ,
		// codigosBarra);

		System.out.println("------------------------- TAREA 34 --------------------------");

	Long timeStar = System.currentTimeMillis();
		List<Factura> listaFacturas = this.facturaService
				.buscarMayoresFecha(LocalDateTime.of(2022, Month.APRIL, 20, 0, 0));
		// listaFacturas.forEach(System.out::println);

		System.out.println("Predicate & Consumer & Supplier");
		List<Factura> li = listaFacturas.stream().filter(f -> f.getDetallesFactura().size() > 2)
				.filter(fa -> fa.getCliente().getCedula().equals("1600102130")).limit(1).collect(Collectors.toList());

		li.forEach(System.out::println);
		
		System.out.println("Se utilizo una interfaz Predicate para filtrar datos & Consumer & Predicate");
		List<DetalleFactura> sd = li.get(0).getDetallesFactura();		
		List<DetalleFactura> df2 = sd.stream().filter(d->d.getSubtotal().compareTo(new BigDecimal(0.50))>0).collect(Collectors.toList());
		df2.forEach(System.out::println);
		
		
		System.out.println("Reporte de factura usando una clase DTO: Function & Predicate & Supplier & consumer");
		listaFacturas.stream().filter(fc ->fc.getCliente().getCedula().equals("1600102130")).map(f->{
			FacturaDTO fac = new FacturaDTO();
		  fac.setNombre(f.getCliente().getNombre());
			fac.setApellido(f.getCliente().getApellido());
			fac.setCedula(f.getCliente().getCedula());
			fac.setFecha(f.getFecha());
			fac.setMontoTotal(f.getMontoTotal());
			fac.setNumero(f.getNumero());
			return fac;
		})
		.limit(1)
		...
		long tiempoFinal = (timeEnd - timeStar)/100;
		
		System.out.println("Tiempo de ejecucion: "+tiempoFinal +" segundos");
